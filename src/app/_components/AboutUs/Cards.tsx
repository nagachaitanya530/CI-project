    ...
    <section className="bg-[#1D438D] py-10 sm:py-14 rounded-3xl overflow-hidden w-full">
      ...
      <div className="relative overflow-x-hidden w-full max-w-screen-xl mx-auto px-4 sm:px-6">
            ...
            {timelineData.map((item, index) => (
              <SwiperSlide key={index}>
                <div className="bg-white rounded-xl shadow p-4 h-full flex flex-col">
                  <div className="text-center text-blue-800 text-lg sm:text-xl font-bold mb-2">
                    {item.year}
                  </div>
                  <img
                    src={item.image}
                  ...
                  />
                  <p className="text-black text-center mt-3 text-sm sm:text-base">
                    {item.title}
                  </p>
                </div>
              </SwiperSlide>
        ...
        <div className="flex justify-center gap-4 mt-6">
          ...
          >
            <ChevronLeft className="text-blue-800 w-5 h-5 sm:w-6 sm:h-6" />
          </button>
          <button
            ref={nextRef}
            className="bg-white p-2 rounded-full shadow hover:bg-gray-100"
          >
            <ChevronRight className="text-blue-800 w-5 h-5 sm:w-6 sm:h-6" />
          </button>
        </div>
      </div>
    </section>
