    ...
    <section className="relative max-w-6xl mx-auto px-2 py-10">
      <div className="flex flex-col relative z-10">
        {steps.map((step, idx) => (
          <div
            key={idx}
            className={`flex flex-col items-center w-full ${idx !== steps.length - 1 ? "mb-5" : ""
              }`}
          >
            {/* Each Step */}
            <div
              className={`flex flex-col md:flex-row items-center gap-16 w-full ${idx % 2 === 1 ? "md:flex-row-reverse" : ""
                }`}
            >
              {/* Step Image */}
              <motion.div
