function RelatedArticle(){
    return(
        <>
        <div className="comm-section mb-25  aos-init aos-animate" data-aos="fade-in" data-duration="50">
                <div className="container">
                    <h3 className="sec-title text-center mb-7 text-5xl">
                        {" "}
                        <span className="highlighter text-orange-600 font-bold">Related Articles on Studying</span>  In France
                    </h3>
                    <div className="related-articles-a border-2 border-gray-200 mx-15 rounded-xl p-5 text-blue-900 ">
                        <p>
                            <a href="" target="_blank">
                                Studying &amp; Living in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Masters in France for Indian Students
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                France Scholarships for Indian Students
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Benefits Of Studying In France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Best Cities to Study in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Studying Culinary Arts in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                MBA in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Fashion Designing Courses in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Hospitality and Tourism Education in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Engineering in France
                            </a>
                            |&nbsp;
                            <a href="" target="_blank">
                                Study Information Technology in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Study in France Consultants
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Employment Opportunities for Indian Students in France
                            </a>
                            |&nbsp;
                            <a href="" target="_blank">
                                France Virtual Admission Day-Jun 2025
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Study PhD in France
                            </a>
                            |&nbsp;
                            <a href="" target="_blank">
                                Universities in France
                            </a>{" "}
                            | &nbsp;
                            <a href="" target="_blank">
                                Students life in France
                            </a>
                        </p>
                    </div>
                </div>
            </div>
        </>
    )
}
export default RelatedArticle;